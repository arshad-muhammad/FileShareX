...
async function calculateFileSHA256(file) {
  ...
  if (window.crypto && window.crypto.subtle) {
    try {
      ...
      console.warn(
        "Native Web Crypto SHA-256 failed, falling back to JS implementation:",
        e,
      );
    }
  }
  ...
  socket.on("message-history", (messages) => {
    ...
    if (messages && messages.length > 0) {
      ...
      appendSystemAnnouncement(
        "Welcome to #" +
          state.activeChannelName.innerText +
  ...
  socket.on("message", (msg) => {
    const isNearBottom =
      DOM.messagesContainer.scrollHeight -
        DOM.messagesContainer.scrollTop -
        DOM.messagesContainer.clientHeight <
      150;
    appendMessage(msg);
...
function appendMessage(msg) {
  ...
  const avatarCol = isSelf
    ? state.color
    : getUserColor(msg.username) || "#cbd5e1";
  ...
  if (msg.type === "text") {
    ...
    if (isImage) {
      ...
      div.addEventListener("click", () =>
        openPreviewModal(msg.fileName, msg.fileUrl, msg.fileType),
      );
...
async function queueAndUploadFiles(filesList) {
  ...
  for (const file of filesList) {
    ...
    try {
      ...
      task.status = "error";
      task.errorMessage = err.message || "Checksum verification failed";
      renderUploadTasks();
    }
  }
}
...
async function runChunkedUpload(uploadId) {
  ...
  try {
    ...
    if (!statusRes.ok) {
      ...
      throw new Error(
        `Upload status failed (${statusRes.status}): ${statusText}`,
      );
    }

    const statusData = await statusRes.json();
    task.uploadedChunks = Array.isArray(statusData.uploadedChunks)
    ...
    for (let chunkIndex = 0; chunkIndex < task.totalChunks; chunkIndex++) {
      ...
      if (!chunkRes.ok) {
        ...
        throw new Error(
          `Chunk upload failed (${chunkRes.status}): ${chunkText}`,
        );
      }
      ...
      if (task.bytesUploaded > task.fileSize)
      ...
      if (completeRes.ok && completeData.success) {
        ...
        throw new Error(
          completeData.error ||
            ...
            `Server assembly failure (${completeRes.status})`,
        );
      }
    }
  } catch (err) {
  ...
  state.activeUploads.forEach((task, id) => {
    ...
    li.innerHTML = `
      ...
      ${
        task.status === "error"
          ? `
        <div class="upload-task-error">Error: ${escapeHTML(task.errorMessage || "Upload failed")}</div>
      `
          : ""
      }
    ...
    if (resumeBtn)
      resumeBtn.addEventListener("click", () => resumeUploadTask(id));

    const cancelBtn = li.querySelector(".cancel-btn");
    if (cancelBtn)
      cancelBtn.addEventListener("click", () => cancelUploadTask(id));
...
function updateUploadBadgeCount() {
  const activeCount = Array.from(state.activeUploads.values()).filter(
...
function setupEventListeners() {
  ...
  DOM.usernameForm.addEventListener("submit", (e) => {
    ...
    if (selectedUsername) {
      ...
      DOM.currentUserAvatar.innerText = selectedUsername
        .charAt(0)
        .toUpperCase();
  ...
  DOM.dragDropZone.addEventListener("dragleave", (e) => {
    ...
    if (
      ...
      !DOM.dragDropZone.contains(e.relatedTarget)
    ) {
  ...
  DOM.searchInput.addEventListener("input", () => {
    ...
    if (q) {
      ...
      socket.emit("search-messages", { query: q }, (results) => {
        ...
        DOM.messagesContainer.innerHTML = "";
        appendSystemAnnouncement(
          `Search results for "${q}" inside this channel (${results.length} found):`,
        );
